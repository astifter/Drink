...
static void initialise_ui(void) {
  ...
  s_res_image_right = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_RIGHT);
  // s_textlayer_1
  ...
  action_bar_layer_set_icon(s_actionbarlayer_1, BUTTON_ID_SELECT, s_res_image_right);
...
static void destroy_ui(void) {
  ...
  gbitmap_destroy(s_res_image_right);
}
...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
  if (selected_time_index == 1) {
    ...
    action_bar_layer_set_icon(s_actionbarlayer_1, BUTTON_ID_SELECT, s_res_image_left);
...
void show_time_selector(struct tm initial, TimeSelected c) {
  ...
  selected_time_index = 1;
  initialise_ui();
  s_res_image_left = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_LEFT);
